...
**/
	...
	while(1){
		if(temp->Cont != Content){
			if(temp->Cont < Content){
				if (temp->RPtr == NULL){
					temp->RPtr = malloc(sizeof(struct tBTNode));
					temp->RPtr->Cont = Content;
					temp->RPtr->LPtr = NULL;
					temp->RPtr->RPtr = NULL;
					break;
				}
				...
				if (temp->LPtr == NULL){
					temp->LPtr = malloc(sizeof(struct tBTNode));
					temp->LPtr->Cont = Content;
					temp->LPtr->LPtr = NULL;
					temp->LPtr->RPtr = NULL;
					break;
				}
...
**/
	...
	while(ptr != NULL){ //dokud nemam konec
		SPushP(Stack, ptr);
		BTWorkOut(ptr);
		ptr = ptr->LPtr;//neni nejlevnejsi tak posouvam pointer doleva
	}
...
**/
	...
	while(SEmptyP(&zasobnik) == FALSE){
			temp = STopPopP(&zasobnik);
			Leftmost_Preorder(temp->RPtr, &zasobnik);
	}
}
...
**/
	...
	while (SEmptyP(&zasobnik) == false){
		temp = STopPopP(&zasobnik);
		BTWorkOut(temp);
		Leftmost_Inorder(temp->RPtr, &zasobnik);
	}
}
...
**/
	...
	while(SEmptyP(&zasobnikP) == FALSE){
		RootPtr = STopPopP(&zasobnikP);
		SPushP(&zasobnikP, RootPtr);
		if(STopPopB(&zasobnikB)){
			SPushB(&zasobnikB, false);
			Leftmost_Postorder(RootPtr->RPtr, &zasobnikP, &zasobnikB);
		}
		else{
			STopPopP(&zasobnikP);
			BTWorkOut(RootPtr);
...
**/
	...
	while((*RootPtr) != NULL || !SEmptyP(&zasobnikP)){
		if ((*RootPtr) == NULL) {
			if (SEmptyP(&zasobnikP) == false) {
				(*RootPtr) = STopPopP(&zasobnikP);
			}
		}
		else{
			if ((*RootPtr)->RPtr != NULL){
				SPushP(&zasobnikP, (*RootPtr)->RPtr);
			}
			temp = *RootPtr;
