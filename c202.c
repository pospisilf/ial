...
*/
	...
	if(s != NULL){ //Jestliže zadaný parametr NENÍ nulový
		s->top = -1; //Přiřaď hodnotu
	}
	else{ //Jestliže zadaný parametr JE nulový
		stackError(SERR_INIT); //Volej funkci stackError
	}	
}
...
*/
	  ...
	  return (s->top != -1) ? 0 : 1; //Overuji hodnotu vrchu zasobniku, kdyz je zasobnik prazdny, vracim 1, jinak 0
}
...
*/
	...
	return ((s->top+1)<STACK_SIZE) ? 0: 1; //jestli je vrchol zasobniku vetsi nez kapacita, vracime 1, jinak 0
}
...
*/
	...
	if(stackEmpty(s) == -1){ //jestlize je zasobnik prazdny (hodnota -1)
		stackError(SERR_TOP);	//vypuis error
	}
	else{	//jestli v nem neco je
		*c=s->arr[s->top]; //do c ulozme hodnotu z vrchu zasobniku
	}
}
...
** 
*/
	if(stackEmpty(s) == 0){ //jestli neni zasobnik prazdny
		s->top--; // odstran hodnotu z vrcholu zasobniku
	}
}
